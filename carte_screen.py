...
class CarteScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # NAVBAR identique à EquipementScreen
        ...
        for name, screen_name in buttons:
            btn = Button(
                text=name if name != "Carte" else f"[b]{name}[/b]",
                markup=(name == "Carte"),
                background_color=(0, 0.4, 0.6, 1),
                size_hint=(1, 1),
                color=(1, 1, 1, 1) if name != "Carte" else (1, 0.6, 0, 1)
            )
            btn.bind(on_press=lambda instance, s=screen_name: setattr(self.manager, 'current', s))
        ...
        # CONTENU PRINCIPAL
        content = BoxLayout(size_hint=(1, 0.9))
